...
begin
  ...
  clock_enable_0 : clock_enable port map (clk => clk, btnU => btnU, btnC => btnC, enable => enable);

  -- fetch memory content, using given module get_mem.vhd       
  get_mem_0 : get_mem port map (clk => clk , enable => enable, data => data, upper_lower => upper_lower);
